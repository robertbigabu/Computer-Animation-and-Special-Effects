...
constexpr const char* studentID = "314552038";
...
gfx::Rigidbody* ElevatorTerrainGraphics = nullptr;


std::unique_ptr<gfx::Plane> g_Plane = nullptr;
std::unique_ptr<gfx::SoftJelly> g_jelly = nullptr;
std::unique_ptr<gfx::Elevator> g_elevator = nullptr;
...
// Switch for render Scene panel
bool isUsingScenePanel = false;
// for simulation
...
int main() {
    ...
    // Load data from assets
    {
        ...
        auto jelly1 = std::make_shared<gfx::Texture>(pf::find("Texture/slime.png"));
        auto jelly2 = std::make_shared<gfx::Texture>(pf::find("Texture/slime.png"));
        auto jelly3 = std::make_shared<gfx::Texture>(pf::find("Texture/slime.png"));
        auto jelly4 = std::make_shared<gfx::Texture>(pf::find("Texture/slime.png"));
        auto jelly5 = std::make_shared<gfx::Texture>(pf::find("Texture/slime.png"));
        auto jelly6 = std::make_shared<gfx::Texture>(pf::find("Texture/slime.png"));
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        switch (sceneIdx) {
            case 0:
                ...
                break;
            case 1:
                ElevatorTerrainGraphics->setModelMatrix(particleSystem.elevatorTerrain->getModelMatrix());
                ElevatorTerrainGraphics->render(&shadowProgram);
                break;
        }
        ...
        switch (sceneIdx) {
            case 0:
                ...
                break;
            case 1:
                ElevatorTerrainGraphics->render(&renderProgram);
                break;
        }
...
GLFWwindow* initialize() {
    ...
    int framerate = int(1 / particleSystem.deltaTime);
    simulationPerFrame = framerate / vidMode->refreshRate + static_cast<bool>(framerate % vidMode->refreshRate);
    ...
    g_elevator = std::make_unique<gfx::Elevator>();
    //g_elevator->setTexture(wood);
    // Physics engine
    auto PlaneTerrain = simulation::TerrainFactory::CreateTerrain(simulation::TerrainType::Plane);
    auto ElevatorTerrain = simulation::TerrainFactory::CreateTerrain(simulation::TerrainType::Elevator);
    ...
    ElevatorTerrainGraphics = g_elevator.get();
    ElevatorTerrainGraphics->setModelMatrix(ElevatorTerrain->getModelMatrix());
    particleSystem.setTerrain(std::move(ElevatorTerrain), "ELEVATOR");
    return window;
}
...
void mainPanel() {
    // Main Panel
    ImGui::SetNextWindowSize(ImVec2(220.0f, 150.0f), ImGuiCond_Once);
    ...
    if (ImGui::Begin("Main Control Panel")) {
        ...
        // if (ImGui::Button("Scene Panel")) {
        //     isUsingScenePanel ^= true;
        // }
    }
    ImGui::End();
}
...
void recordingPanel() {
    ...
    ImGui::SetNextWindowCollapsed(0, ImGuiCond_Once);
    ImGui::SetNextWindowPos(ImVec2(60.0f, 210.0f), ImGuiCond_Once);
    ImGui::SetNextWindowBgAlpha(0.2f);
    if (ImGui::Begin("Recording Control Panel", &isUsingRecordingPanel)) {
        ...
        if (ImGui::Button(recordingText)) {
            if (isRecording) {
                g_Exporter->exportVideo(int(1 / (simulationPerFrame * particleSystem.deltaTime)));
            } else {
            }
...
void simulationPanel() {
    ...
    if (ImGui::Begin("Simulation Control Panel", &isUsingSimulationPanel)) {
        if (ImGui::Button(particleSystem.isSimulating ? "Stop" : "Start")) {
            ...
            particleSystem.elevatorTerrain->reset();
            //startTime = std::chrono::steady_clock::now();
            particleSystem.elevatorCounter = 0;
        ...
        if (particleSystem.isSimulating) {
            ...
            if (ImGui::Button("POSITION_BASED_DYNAMIC")) {
                particleSystem.setIntegrator(IntegratorFactory::CreateIntegrator(IntegratorType::PositionBasedDynamic));
            }
...
void debugPanel() {
    ...
    ImGui::SetNextWindowCollapsed(0, ImGuiCond_Once);
    ImGui::SetNextWindowPos(ImVec2(60.0f, 420.0f), ImGuiCond_Once);
    ImGui::SetNextWindowBgAlpha(0.2f);
...
void scenePanel() {
    ...
    ImGui::SetNextWindowCollapsed(0, ImGuiCond_Once);
    ImGui::SetNextWindowPos(ImVec2(60.0f, 300.0f), ImGuiCond_Once);
    ImGui::SetNextWindowBgAlpha(0.2f);
    if (ImGui::Begin("Scene Panel", &isUsingScenePanel)) {
        if (ImGui::Button("Slope")) {
            sceneIdx = 0;
            particleSystem.sceneIdx = 0;
            particleSystem.reset();
            particleSystem.isSimulating = false;
            g_jelly->update();
            particleSystem.elevatorTerrain->reset();
        ...
        ImGui::SameLine();
        if (ImGui::Button("Elevator")) {
            sceneIdx = 1;
            particleSystem.sceneIdx = 1;
            particleSystem.reset();
            particleSystem.isSimulating = false;
            g_jelly->update();
            particleSystem.elevatorTerrain->reset();
...
void renderUI(GLFWwindow* window) {
    ...
    if (isUsingScenePanel) scenePanel();
    ImGui::Render();
