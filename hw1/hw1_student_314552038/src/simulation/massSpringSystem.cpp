...
void MassSpringSystem::setTerrain(std::unique_ptr<Terrain>&& terrain, std::string planeType) {
    ...
    if (planeType == "ELEVATOR") {
        this->elevatorTerrain = std::move(terrain);
    }
...
void MassSpringSystem::computeJellyForce(Jelly& jelly) {
    ...
    switch (sceneIdx) {
        case 0:
            ...
            break;
        case 1:
            elevatorTerrain->handleCollision(deltaTime, jelly);
            break;
    }
}

void MassSpringSystem::computeElevatorForce() {

    int cycle = elevatorCounter % 4002;
    if (cycle >= 0 && cycle<=1999) {
        elevatorTerrain->addForce(Eigen::Vector3f(0, 3, 0));
    } else if (cycle == 2000) {
        elevatorTerrain->setVelocity(Eigen::Vector3f(0, 0, 0));
    } else if (cycle >= 2001 && cycle <= 4000) {
        elevatorTerrain->addForce(Eigen::Vector3f(0, -3, 0));
    } else if (cycle == 4001) {
        elevatorTerrain->setVelocity(Eigen::Vector3f(0, 0, 0));
    }
